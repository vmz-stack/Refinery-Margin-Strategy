# Investigating Whether Process-Simulation-Derived Weights Improve Crack Spread Mean-Reversion Strategies

> An empirical investigation into whether product yield weights derived from an Aspen HYSYS atmospheric crude distillation simulation improve the risk-adjusted performance of a crack spread mean-reversion strategy, relative to the generic 3:2:1 textbook ratio. The answer is not obvious — and that's the point.

---

## The Research Question

Most crack spread models use a fixed 3:2:1 product weighting — two barrels gasoline, one barrel heating oil, against three barrels crude. This ratio is an industry convention, not an engineering derivation:

```python
Crack Spread = (2/3 × Gasoline + 1/3 × Heating Oil) − Crude
```

This project asks: **does replacing that arbitrary ratio with yield weights derived from an actual Aspen HYSYS CDU simulation of a WTI Light crude assay produce a statistically better mean-reversion strategy?**

The HYSYS-derived spread formula:

```python
Spread = (yield_naphtha × RBOB × 42) + (yield_diesel × HO × 42) − (priced_yield_fraction × WTI) − utility_cost_per_bbl
```

Where `yield_naphtha = 0.0342` and `yield_diesel = 0.1223` come directly from a converged Aspen HYSYS atmospheric CDU simulation of a real WTI Light crude assay (ExxonMobil EMTEC, Reference WTIL220Y, 2020). The utility cost per barrel ($0.112/bbl) comes from the HYSYS Activated Economics module.

The methodology:
1. Simulate a real crude assay in HYSYS to extract physically-grounded yield fractions
2. Construct a partial product spread using only the two HYSYS outputs with liquid futures proxies (naphtha → RBOB, diesel → Heating Oil)
3. Apply an identical OU mean-reversion framework to both the HYSYS-weighted spread and the generic 3:2:1 spread
4. Compare out-of-sample risk-adjusted performance

**Finding:** Out-of-sample Sharpe ratios are broadly comparable between the two approaches. The HYSYS-derived weighting does not produce a clear, consistent improvement in risk-adjusted returns over the generic 3:2:1 convention in this sample period. This is a valid and informative research outcome — it suggests that for this crude grade and this time period, the additional physical granularity in the weighting does not translate into a statistically meaningful trading edge, though it does produce a spread that is more economically interpretable than the textbook convention.
...
The primary finding is that out-of-sample Sharpe ratios are broadly similar between the HYSYS strategy and the 3:2:1 benchmark. This is reported transparently — the project is framed as a research investigation, and a neutral result is a valid, publishable outcome. The value of the HYSYS approach is not that it definitively generates more alpha, but that it produces a spread with a cleaner economic interpretation: the crude cost is proportional to the fraction of the barrel actually being priced, rather than charging the full barrel cost against a small subset of outputs.
